...
export default function DistrictPopularityChart({ data }) {
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-surface/90 border border-white/10 backdrop-blur-xl px-4 py-3 rounded-2xl shadow-card flex flex-col gap-1">
          <p className="text-xs font-bold text-white">{item.name} District</p>
          <div className="border-t border-white/5 my-1"></div>
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[11px]">
            <span className="text-textMuted">Leaderboard Score:</span>
            <span className="font-bold text-primary text-right">{item.score}</span>
            ...
            <span className="text-textMuted">Trips:</span>
            <span className="font-bold text-white text-right">{item.trips}</span>
  ...
  return (
    <div className="w-full h-80 bg-surface/30 backdrop-blur-xl border border-white/10 rounded-3xl p-6 shadow-md">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h4 className="font-display text-sm font-black text-white uppercase tracking-wider">District Heat Leaderboard</h4>
          <p className="text-[10px] text-textMuted mt-0.5">Engagement index across districts (Views, Saves, Reviews, Trips)</p>
        </div>
      </div>
      <div className="w-full h-[calc(100%-3rem)]">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            <Bar
              dataKey="score"
              fill="#60a5fa" // District theme color
              radius={[6, 6, 0, 0]}
              maxBarSize={35}
            />
          </BarChart>
